...
public:
	...
	MyVector(const MyVector<T>& other) :m_size(other.m_size), m_capacity(other.m_capacity)
	{
	...
	MyVector<T>& operator=(MyVector<T> other) // 1. 关键改动：参数从 const MyVector& 变为 MyVector
	{
...
template <typename T>
void print_vector_state(const char* name, const MyVector<T>& vec) {
	std::cout << name << " -> "
		<< "Size: " << vec.size()
...
int main()
{
	...
	MyVector<int> vec2 = vec1; // 这里会调用拷贝构造函数
